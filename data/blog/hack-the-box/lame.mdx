...
# [CHECK OUT THE VIDEO TUTORIAL](https://www.youtube.com/watch?v=qDNVy6n9Jds)
...
First of all, we verify that we are on the same network as the victim machine by sending a data packet to the IP 10.10.10.3 using the ping command.

```js
  ping -c 4 10.10.10.3
```

- `-c` : Number of ICMP (Internet Control Message Protocol) echo request packets that will be sent.

![Ping](/static/images/htb/Lame/ping.png)

### Nmap

Then, we perform the port scan.
